...
def rest2TGround(Iloc, Irem, S1=0.3, S2=0.5, BP=8, sigma=0):
	"""The rest2TGround function calculates the restraint current for a line
	differental application using the set characteristic along with the local and
	...
	if Iloc ** 2 < BP ** 2:
		LocRestSq = Iloc ** 2 / 2 * S1 ** 2
	else:
		LocRestSq = BP ** 2 + (Iloc ** 2 - 2 * (S1 * BP) ** 2) / (2 * S2 ** 2)

	if Irem ** 2 < BP ** 2:
		RemRestSq = Irem ** 2 / 2 * S1 ** 2
	else:
		RemRestSq = BP ** 2 + (Irem ** 2 - 2 * (S1 * BP) ** 2) / (2 * S2 ** 2)

	return np.sqrt(max(LocRestSq, RemRestSq))


def GndTrip(diff, rest, P=0.2):
	"""The trip decision is made by passing in the differential and restraint
	currents along with the basic pickup. If the Severity calculation is > 0 a
	trip is declared
	"""
	SA = diff ** 2 - (2 * P**2 + rest ** 2)
	if SA > 0:
		out = 'Trip'
	else:
		out = 'No Op'
